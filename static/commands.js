...
async function _runManualCompression(focusTopic){
  ...
  try{
    ...
    try{
      ...
      if(!S.session||S.session.session_id!==sid) return;
      // Metadata-only preflight must not replace the visible transcript or its
      // pagination/tool window with the intentionally empty response arrays.
      S.session={...S.session,...live.session,messages:S.messages,tool_calls:S.toolCalls};
    }catch(preflightErr){
      if(typeof clearCompressionUi==='function') clearCompressionUi();
      if(typeof _setCompressionSessionLock==='function') _setCompressionSessionLock(null);
      if(typeof setBusy==='function') setBusy(false);
...
async function cmdRetry(){
  ...
  try{
    ...
    if(!S.session||S.session.session_id!==activeSid)return;
    if(data&&data.session){
      S.session=data.session;S.messages=data.session.messages||[];S.toolCalls=data.session.tool_calls||[];
      if(typeof clearLiveToolCards==='function')clearLiveToolCards();
      if(typeof _messagesTruncated!=='undefined')_messagesTruncated=!!data.session._messages_truncated;
      if(typeof _oldestIdx!=='undefined')_oldestIdx=data.session._messages_offset||0;
      renderMessages();
